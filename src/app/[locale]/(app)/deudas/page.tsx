...
export default function DeudasPage() {
  ...
  async function handleDelete(debt: DebtDto) {
    const ok = await confirm(t("confirmDelete"), t("confirmDeleteMsg", { name: debt.name }));
    if (!ok) return;
    try {
  ...
  return (
    ...
    <div className="animate-fade-in">
              ...
              {summary.paid.map((debt) => (
                <Card key={debt.id} className="opacity-60">
                  <div className="flex items-center gap-3">
                    ...
                    <Badge tone="positive">{t("paidBadge")}</Badge>
                    <button
      ...
      <Modal
        ...
        title={editing ? t("editDebt") : t("newDebtTitle")}
      >
        <div className="space-y-4">
          <div>
            <Label htmlFor="debt-name">{t("common.name")}</Label>
            <Input
              id="debt-name"
              value={name}
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <Label htmlFor="debt-total">{t("totalAmount")}</Label>
              <Input
                id="debt-total"
                type="number"
                step="0.01"
            ...
            <div>
              <Label htmlFor="debt-paid">{t("paidAmount")}</Label>
              <Input
                id="debt-paid"
                type="number"
                step="0.01"
    ...
    <Card>
      <button onClick={() => onEdit(debt)} className="w-full text-left">
        <div className="flex items-start justify-between gap-3">
          ...
          {overdue ? (
            <Badge tone="negative">{t("overdue")}</Badge>
          ) : pct >= 100 ? (
