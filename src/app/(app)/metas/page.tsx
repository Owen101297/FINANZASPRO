...
export default function MetasPage() {
  ...
  async function handleSave() {
    if (!name.trim() || !targetAmount || Number(targetAmount) <= 0) {
      toast("Completa nombre y meta", "error");
      return;
    }
    setSaving(true);
    try {
  ...
  return (
    ...
    <div className="animate-fade-in">
      <header className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-extrabold tracking-tight">Metas de ahorro</h1>
          <p className="mt-0.5 text-sm text-muted-foreground">
            {completedGoals.length} completada{completedGoals.length === 1 ? "" : "s"} ·{" "}
            {activeGoals.length} en camino
          </p>
        </div>
        <button
          onClick={openCreate}
      ...
      {isLoading ? (
              ...
              {completedGoals.map((goal) => (
                <Card key={goal.id} className="border-positive/30 bg-positive/5 opacity-80">
                  <div className="flex items-center gap-3">
                    ...
                    <Badge tone="positive">¡lograda!</Badge>
                  </div>
                </Card>
              ))}
            </>
        ...
        <Card>
          <EmptyState
            ...
            hint='Ej.: "Fondo de emergencia $1.000". Define cuánto quieres ahorrar y ve tu progreso.'
          />
        </Card>
      )}
