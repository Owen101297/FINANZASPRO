...
export default function CuentasPage() {
  ...
  async function handleSave() {
    if (!name.trim()) {
      toast("Ingresa un nombre para la cuenta", "error");
      return;
    }
    setSaving(true);
    try {
  ...
  return (
    <div className="animate-fade-in">
      <header className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-extrabold tracking-tight">Cuentas</h1>
          <p className="mt-0.5 text-sm text-muted-foreground">Dónde guarda tu dinero</p>
        </div>
        <button
          onClick={openCreate}
      ...
      {isLoading ? (
              ...
              <div className="flex items-center gap-3.5">
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    ...
                    {account.archived && <Badge>archivada</Badge>}
                  </div>
        ...
        <Card>
          <EmptyState
            ...
            title="Sin cuentas"
            hint="Ejemplo: Efectivo, Banco, Tarjeta. Crea la primera para empezar."
          />
        </Card>
      )}
